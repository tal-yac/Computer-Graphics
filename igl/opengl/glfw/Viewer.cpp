...
IGL_INLINE bool Viewer::load_mesh_from_data(const Eigen::MatrixXd &V,
  ...
  data()->uniform_colors(
      Eigen::Vector3d(255.0 / 255.0, 255.0 / 255.0, 0.0 / 255.0),
...
Viewer::load_mesh_from_file(const std::string &mesh_file_name_string) {
  ...
  if (last_dot == std::string::npos) {
    std::cerr << "Error: No file extension found in " << mesh_file_name_string
              << std::endl;
    return false;
  }
...
IGL_INLINE void Viewer::Draw(int shaderIndx, const Eigen::Matrix4f &Proj,
    ...
    if (shape->Is2Render(viewportIndx)) {
      ...
      if (!(flgs & 65536)) {
        ...
        if (flgs & 16384) {
          ...
          Update(Proj, View * Normal,
                 Normal.inverse() * Model * scale_mat.matrix(), 0, i);
        } else {
...
int Viewer::AddShader(const std::string &Vertex_Shader,
                      const std::string &Fragment_shader) {
  shaders.push_back(
      new Shader(Vertex_Shader, Fragment_shader, false, next_data_id));
  next_data_id += 1;
  return (shaders.size() - 1);
}
...
int Viewer::AddShapeFromFile(const std::string &fileName, int parent,
                             unsigned int mode, int viewport) {
...
int Viewer::AddShape(int type, int parent, unsigned int mode, int viewport) {
  ...
  if (type == Axis) {
    ...
    data()->add_edges((Eigen::RowVector3d::UnitX() * 4),
                      -(Eigen::RowVector3d::UnitX() * 4),
                      Eigen::RowVector3d(255, 0, 0));
    data()->add_edges((Eigen::RowVector3d::UnitY() * 4),
                      -(Eigen::RowVector3d::UnitY() * 4),
...
int Viewer::AddShapeCopy(int indx, int parent, unsigned int mode,
                         int viewport) {
  load_mesh_from_data(data_list[indx]->V, data_list[indx]->F,
                      data_list[indx]->V_uv, data_list[indx]->F_uv);
...
int Viewer::AddShapeFromData(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                             const Eigen::MatrixXd &UV_V,
                             const Eigen::MatrixXi &UV_F, int type, int parent,
                             unsigned int mode, int viewport) {
...
void Viewer::MouseProccessing(int button, int xrel, int yrel, float movCoeff,
                              Eigen::Matrix4d cameraMat, int viewportIndx) {
  ...
  if (pickedShape <= 0 && !(staticScene & (1 << viewportIndx)))
    ...
    scnMat = (MakeTransd() *
              GetPriviousTrans(Eigen::Matrix4d::Identity(), pickedShape))
                 .inverse();
  else if (pickedShape > 0)
    scnMat =
        (GetPriviousTrans(Eigen::Matrix4d::Identity(), pickedShape)).inverse();
  ...
  if (button == 1) {
    ...
    if (button == 0) {
      //                if (pickedShape > 0 )
      WhenRotate(scnMat * cameraMat, -((float)xrel / 180) / movCoeff,
                 ((float)yrel / 180) / movCoeff);
    } else {
  ...
  // data_list[pickedShape]->MyRotate(Eigen::Vector3d(0, 1, 0), -xrel / 2.0f,
  // 0); MyRotate(Eigen::Vector3d(1, 0, 0), -yrel / 2.0f, 1);
  // //WhenRotate();
  //     pickedShape = -1;
}
...
void Viewer::WhenTranslate(const Eigen::Matrix4d &preMat, float dx, float dy) {
  ...
  if (pickedShape == -1 || data_list[pickedShape]->IsStatic())
    ...
    obj = (Movable *)data_list[pickedShape];
  }
...
void Viewer::WhenScroll(const Eigen::Matrix4d &preMat, float dy) {
  if (pickedShape == -1 || data_list[pickedShape]->IsStatic())
    this->TranslateInSystem(preMat.block<3, 3>(0, 0),
                            Eigen::Vector3d(0, 0, dy));
  else
    data_list[pickedShape]->TranslateInSystem(preMat.block<3, 3>(0, 0),
                                              Eigen::Vector3d(0, 0, dy));
  WhenScroll(dy);
}

int Viewer::AddMaterial(unsigned int texIndices[], unsigned int slots[],
                        unsigned int size) {
  materials.push_back(new Material(texIndices, slots, size));
  return (materials.size() - 1);
}

Eigen::Matrix4d Viewer::GetPriviousTrans(const Eigen::Matrix4d &View,
                                         unsigned int index) {
...
float Viewer::AddPickedShapes(const Eigen::Matrix4d &PV,
                              const Eigen::Vector4i &viewport, int viewportIndx,
  ...
  for (int i = 1; i < data_list.size();
       i++) { // add to pShapes if the center in range
    ...
    if (data_list[i]->Is2Render(viewportIndx) && xpix < right && xpix > left &&
        ypix < bottom && ypix > up) {
      pShapes.push_back(i);
  ...
  if (isFound) {
    Eigen::Vector4d tmp =
        MVP * GetPriviousTrans(Eigen::Matrix4d::Identity(), pickedShape) *
        data_list[pickedShape]->MakeTransd() * Eigen::Vector4d(0, 0, 1, 1);
    return (float)tmp.z();
  } else
    return -1;
}
...
void Viewer::BindMaterial(Shader *s, unsigned int materialIndx) {
  ...
  for (int i = 0; i < materials[materialIndx]->GetNumOfTexs(); i++) {
    ...
    s->SetUniform1i("sampler" + std::to_string(i + 1),
                    materials[materialIndx]->GetSlot(i));
  }
}
...
int Viewer::AddTexture(int width, int height, unsigned char *data, int mode) {
  ...
  if (mode) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE,
                 data); // note GL_RED internal format, to save memory.
  }
  glBindTexture(GL_TEXTURE_2D, 0);
  return (textures.size() - 1);
}

void Viewer::Update_overlay(const Eigen::Matrix4f &Proj,
                            const Eigen::Matrix4f &View,
                            const Eigen::Matrix4f &Model,
                            unsigned int shapeIndx, bool is_points) {
...
void Viewer::SetParent(int indx, int newValue, bool savePosition) {
  ...
  if (savePosition) {
    Eigen::Vector4d tmp = data_list[newValue]->MakeTransd() *
                          (data_list[indx]->MakeTransd()).inverse() *
